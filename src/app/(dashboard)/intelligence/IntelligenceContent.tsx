...
interface Index { name: string; base: number; decimals: number; prefix: string; pctVol: number; }
...
function useLiveIndices() {
  ...
  useEffect(() => {
    ...
    const clockInterval = setInterval(() => { setSecondsAgo(Math.floor((Date.now() - lastUpdateRef.current) / 1000)); }, 1000);
    return () => { clearInterval(tickInterval); clearInterval(clockInterval); };
  }, []);
  const indices = INDEX_SEEDS.map((seed, i) => ({ name: seed.name, value: formatValue(seed.prefix, seed.decimals, ticks[i].value), pct: `${ticks[i].pct >= 0 ? "+" : ""}${ticks[i].pct.toFixed(2)}%`, up: ticks[i].pct >= 0 }));
...
export default function IntelligenceContent() {
  ...
  return (
    <div className="space-y-6">
      <motion.div variants={stagger} initial="hidden" animate="visible">
        <motion.div variants={fadeUp} className="flex items-center justify-between gap-3">
          ...
          <div className="flex flex-shrink-0 items-center gap-1.5 rounded-full border border-amber-500/25 bg-amber-500/10 px-3 py-1.5">
            <span className="relative flex h-2 w-2"><span className="relative inline-flex h-2 w-2 rounded-full bg-amber-400" /></span>
      ...
      <p className="text-[10px] text-gray-400">Market data is simulated for demo purposes and does not reflect real prices.</p>
              ...
              <div key={s.name} className="flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1 h-1 w-full rounded-full bg-white/6">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${s.heat}%` }} transition={{ duration: 0.8, ease: "easeOut", delay: 0.1 }} className={`h-full rounded-full ${s.up ? "bg-green-500" : "bg-red-500"}`} />
                  </div>
                </div>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="rounded-2xl border border-white/8 bg-white/3 overflow-hidden">
            <div className="grid grid-cols-5 border-b border-white/6 px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
              <span className="col-span-2">Company</span><span>Users</span><span>Funding</span><span>Threat</span>
            </div>
            {COMPETITORS.map((c, i) => (
              <motion.div key={c.name} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}
                ...
                <div>
                  <div className="mb-1 h-1.5 w-full max-w-[60px] rounded-full bg-white/6">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${c.threats}%` }} transition={{ duration: 0.7, delay: i * 0.07 + 0.2 }}
                      className={`h-full rounded-full ${c.threats >= 75 ? "bg-red-500" : c.threats >= 50 ? "bg-yellow-500" : "bg-green-500"}`} />
                  </div>
                  <span className={`text-[9px] font-bold ${c.threats >= 75 ? "text-red-400" : c.threats >= 50 ? "text-yellow-400" : "text-green-400"}`}>{c.threat}</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible" className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {TRENDS.map((t, i) => (
            <motion.div key={t.topic} variants={scaleIn} className="flex items-center gap-3 rounded-xl border border-white/8 bg-white/3 p-3">
              ...
              <span className={`text-[10px] font-bold flex-shrink-0 ${t.rising ? "text-green-400" : "text-gray-400"}`}>{t.rising ? "↑" : "→"}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
