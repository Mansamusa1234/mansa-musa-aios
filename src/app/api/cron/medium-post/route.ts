...
<p>That's less than £2 a day for a receptionist who never calls in sick, never takes a lunch break, never misses a call, and works every single day of the year. The ROI is immediate and measurable — our customers see new bookings and captured leads from their very first day.</p>
...
<p>There's no complicated setup, no hardware to install, and no IT expertise required. You get a dedicated phone number, a customised greeting, and a dashboard where you can review every call and booking. It works alongside your existing tools and calendar systems seamlessly.</p>
...
export async function GET(req: Request) {
  ...
  if (!token) {
    return NextResponse.json({ error: "MEDIUM_INTEGRATION_TOKEN not set" }, { status: 500 });
  }
  ...
  if (!meRes.ok) {
    ...
    console.error("[cron/medium-post] failed to fetch /me", meRes.status, text);
    return NextResponse.json({ error: "Failed to fetch Medium user" }, { status: 502 });
  }
  ...
  if (!postRes.ok) {
    ...
    console.error("[cron/medium-post] failed to create post", postRes.status, text);
    return NextResponse.json({ error: "Failed to create Medium post" }, { status: 502 });
  }
