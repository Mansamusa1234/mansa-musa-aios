...
// Runs daily at 9am. Sends 3-wave email sequence to users on trial.
// Day 3 → show the value. Day 7 → social proof + urgency. Day 12 → final push.
...
export async function GET(req: Request) {
  ...
  for (const sub of trialing) {
    ...
    // Wave 1 — Day 3: Show the value
    if (dayIn >= 3 && !trialUpsell1SentAt) {
      await sendEmail(
        user.email,
        `${name}, your AI receptionist is ready 🎙️`,
    ...
    // Wave 2 — Day 7: Social proof + urgency
    ...
    // Wave 3 — Day 12: Final push (2 days left)
    if (dayIn >= 12 && !trialUpsell3SentAt) {
      await sendEmail(
        user.email,
        `⚠️ 2 days left — your AI receptionist goes offline`,
        `<div style="font-family:sans-serif;max-width:580px;margin:0 auto;padding:32px;background:#0a0a1a;color:#e2e8f0">
          ...
          <p style="font-size:16px;line-height:1.7">We're offering one thing before your trial ends:</p>
          <div style="background:#1e1b4b;border:2px solid #7c3aed;padding:20px;border-radius:8px;margin:20px 0;text-align:center">
            ...
            <p style="color:#94a3b8;margin:0 0 16px">Refer one client → earn £30/mo back. Refer 5 → the plan pays for itself.</p>
