...
import { getGovContracts } from "@/lib/quiver/client";
...
export async function GET(req: Request) {
  ...
  try {
    const data = await getGovContracts(ticker);
    return NextResponse.json({ data });
  } catch (err) {
    console.error("[quiver/contracts]", err);
    return NextResponse.json({ error: "Failed to fetch government contract data." }, { status: 502 });
  }
}
