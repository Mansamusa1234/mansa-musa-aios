...
import { getLobbyingData } from "@/lib/quiver/client";
...
export async function GET(req: Request) {
  ...
  try {
    const data = await getLobbyingData(ticker);
    return NextResponse.json({ data });
  } catch (err) {
    console.error("[quiver/lobbying]", err);
    return NextResponse.json({ error: "Failed to fetch lobbying data." }, { status: 502 });
  }
}
