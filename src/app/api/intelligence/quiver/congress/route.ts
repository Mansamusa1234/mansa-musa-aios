...
import { getCongressTrading } from "@/lib/quiver/client";
...
export async function GET(req: Request) {
  ...
  try {
    const data = await getCongressTrading(ticker);
    return NextResponse.json({ data });
  } catch (err) {
    console.error("[quiver/congress]", err);
    return NextResponse.json({ error: "Failed to fetch congressional trading data." }, { status: 502 });
  }
}
